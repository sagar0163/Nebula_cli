...
/**
 * Calculates a safety score for a command from 0 (very safe) to 100 (very dangerous).
 * @param {string} command - The command to score.
 * @returns {number} - Safety score.
 */
export const getSafetyScore = (command) => {
    if (READ_PATTERNS.some(p => p.test(command))) {
        return 0; // Very safe, auto-execution allowed
    }
    
    const SAFE_VERBS = ['get', 'describe', 'find', 'list', 'status', 'help', 'version'];
    if (SAFE_VERBS.some(verb => command.includes(verb))) {
        return 10;
    }

    if (CRITICAL_COMMANDS.some(pattern => pattern.test(command))) {
        return 100; // Critical danger
    }
    ...
    return 50; // Unknown/Manual execution
};
