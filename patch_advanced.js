...
let content = fs.readFileSync('src/commands/advanced-session.js', 'utf8');
...
const healRegex = /async function handleAutoHealingSafe\(command, result\) \{\s*try \{\s*const errorMsg = result\.stderr \|\| 'Unknown error';/m;
if (content.match(healRegex)) {
    const replacement = `async function handleAutoHealingSafe(command, result) {
    try {
...
fs.writeFileSync('src/commands/advanced-session.js', content);
